    ...
    async beforeRegisterNodeDef(nodeType, nodeData, app) {
        if (nodeData.name === "SeamStitchLoader") {
            ...
            nodeType.prototype.onNodeCreated = function () {
                ...
                // input_video bypasses the file decode entirely, but this preview
                // player has no way to play an in-memory tensor - it can only ever
                // show whatever file the dropdown below names. Scrubbing that file
                // and picking trim points from it is misleading once input_video is
                // wired: the trim values get applied to the CONNECTED TENSOR, which
                // may be completely different content at completely different frame
                // indices. This surfaced as a real bug - a generated bridge that
                // ignored the picked first/last frames - so it's flagged loudly
                // rather than silently trusted.
                const inputVideoWarning = document.createElement("div");
                Object.assign(inputVideoWarning.style, {
                    color: "#1a1a1a",
                    background: "#ffcc00",
                    borderRadius: "4px",
                    padding: "6px 8px",
                ...
                inputVideoWarning.textContent =
                    "⚠ input_video is connected - the player/timeline below plays whatever the " +
                    "video dropdown names, NOT the connected tensor. Trim values are applied to the " +
                    "connected tensor, which will not match what you're scrubbing unless that dropdown " +
                    "names the exact same file.";
                container.appendChild(inputVideoWarning);

                node.updateInputVideoWarning = function () {
                    const inp = node.inputs && node.inputs.find((i) => i.name === "input_video");
                    inputVideoWarning.style.display = (inp && inp.link != null) ? "block" : "none";
                };
